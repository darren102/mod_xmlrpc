...
%% set_presence struct[{user, String}, {server, String}, {show, String}, {status, String}, {type, String}] Integer
handler(_State, {call, set_presence, [{struct, AttrL}]}) ->
    [User, Server, Show, Status, Type] = get_attrs([user, server, show, status, type], AttrL),
    JID = jlib:make_jid(User, Server, ""),
    if
       Type == "unavailable" ->
          MPacket = set_user_presence_unavailable(Type, Show, Status);
       true ->
          MPacket = set_user_presence_available(Show, Status)
    end,
    R = case get_roster(User, Server) of
        {ok, Roster} ->
            _ = send_user_presence_packets(Roster, MPacket, JID),
	    0;
	{error, Reason} ->
	    io:format("Can't get roster of user ~p@~p: reason: ~p~n", [User, Server, Reason]),
	    1;
	{badrpc, Reason} ->
	    io:format("Can't get roster of user ~p@~p: ~p~n", [User, Server, Reason]),
	    1
    end,
    {false, {response, [R]}};
...
%% Set Presence
%% -----------------------------
set_user_presence_available(Show, Status) ->
    {xmlelement, "presence",
     [],
...
set_user_presence_unavailable(Type, Show, Status) ->
    {xmlelement, "presence",
     [{"type", Type}],
     [{xmlelement, "show", [], [{xmlcdata, Show}]},
      {xmlelement, "status", [], [{xmlcdata, Status}]}
...
%% Sending the presence to each user in your roster
send_user_presence_packets(Roster, Packet, JID) ->
    lists:foldl(
      fun(Item, _) ->
         JIDS = jlib:jid_to_string(Item#roster.jid),
         SJID = jlib:string_to_jid(JIDS),
         ejabberd_router:route(JID, SJID, Packet)
      end,
      [],
      Roster).
